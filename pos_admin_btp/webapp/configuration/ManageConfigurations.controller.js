sap.ui.define([
	...
	return BaseController.extend("com.posadminbtp.configuration.ManageConfigurations", {
		...
		async fetchMasterData() {
			const paths = ["authendication_methods", "destination_types", "proxy_types"],
				prefixPath = "local_service/mockdata/",
				ext = ".json";

			let collData = await Promise.all(paths.map(async (e) => {
				let path = prefixPath + e + ext;
				const res = await fetch(URLConstants.URL.app_end_point + path);
				const json = await res.json();
				this.getView().setModel(new JSONModel(json), e);
			}));
		...
		onListItemPress(oEvent) {
			let configCtxt = oEvent.getSource().getSelectedItem().getBindingContext();
		...
		handleStateChange: function (oEvt) {
			...
			const aCells = oState.Columns.map(function (oColumnState) {
				...
				} else if (oColumnState.key === 'type_col') {
					cell = new sap.m.Text({
                        text: { path: colPath, formatter: this.typeFormatter.bind(this)}
                    });
				}  else if (oColumnState.key === 'system_type_col') {
					cell = new sap.m.Text({
